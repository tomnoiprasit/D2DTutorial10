...
HRESULT BaseApp::Initialize()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		m_hwnd = CreateWindow(
			...
			"DubboIT - Scene Larger Than View",
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
...
HRESULT BaseApp::OnRender()
{
	...
	m_pD2DHRT->FillRectangle(D2D1::RectF(obj.GetX()*GRID_SIZE, 
										obj.GetY()*GRID_SIZE, 
										obj.GetX()*GRID_SIZE + obj.GetWidth(), 
										obj.GetY()*GRID_SIZE + obj.GetWidth()), 
										m_pD2DSolidBrush03);
	m_pD2DHRT->DrawRectangle(D2D1::RectF(obj.GetX()*GRID_SIZE, 
										obj.GetY()*GRID_SIZE, 
										obj.GetX()*GRID_SIZE + obj.GetWidth(), 
										obj.GetY()*GRID_SIZE + obj.GetWidth()), 
										m_pD2DSolidBrush01, 1, NULL);

	m_pD2DHRT->FillRectangle(D2D1::RectF(objNonMoving.GetX()*GRID_SIZE,
		objNonMoving.GetY()*GRID_SIZE,
		objNonMoving.GetX()*GRID_SIZE + objNonMoving.GetWidth(),
		objNonMoving.GetY()*GRID_SIZE + objNonMoving.GetWidth()),
		m_pD2DSolidBrush05);
	m_pD2DHRT->DrawRectangle(D2D1::RectF(objNonMoving.GetX()*GRID_SIZE,
		objNonMoving.GetY()*GRID_SIZE,
		objNonMoving.GetX()*GRID_SIZE + objNonMoving.GetWidth(),
		objNonMoving.GetY()*GRID_SIZE + objNonMoving.GetWidth()),
		m_pD2DSolidBrush01, 1, NULL);
...
HRESULT BaseApp::OnKeyDown(WPARAM wparam) {
	...
	switch (wparam) {
	case VK_LEFT:
		obj.SetPosition(obj.GetX() - 1,obj.GetY());
		break;
	case VK_RIGHT:
		obj.SetPosition(obj.GetX() + 1, obj.GetY());
		break;
